    ...
    <div class="single-plugin-overview">
      <div class="single-plugin-detail">
        <div class="single-plugin-top-bar">
          <h4>{{ $page.files.label }} - <span>{{ $page.files.description }}</span></h4>
        </div>
        <div class="single-plugin-main">
          ...
          <p>The {{ $page.files.name }} <a href="https://docs.meltano.com/concepts/plugins#file-bundles">file bundle</a>
            {{ $page.files.definition }}</p>
          ...
          <ol>
            <li>Add the {{ $page.files.name }} file to your project using
              <pre class="inline-code-block"><code>meltano add</code></pre>:
            </li>
            <pre><code>meltano add file {{ $page.files.name }}</code></pre>
          </ol>
          <h3>Next steps</h3>
          ...
          <h2>Capabilities</h2>
          <p>The current capabilities for <pre class="inline-code-block"><code>{{ $page.files.name }}</code></pre> may have been automatically set when
          originally added to the Hub. Please review the capabilities when using this extractor. If you find they are
          out of date, please consider updating them by making a pull request to the YAML file that defines the
          capabilities for this extractor.</p>
          <p>This plugin has the following capabilities:</p>
          <ul>
            <li v-for="(capability, index) in $page.files.capabilities" v-bind:key="index">{{ capability }}</li>
          </ul>
          <h2>Settings</h2>
          <p>{{ $page.files.name }} requires the configuration of the following settings:</p>
          <ul>
            <li v-for="(setting, index) in $page.files.settings" v-bind:key="index">{{ setting.name }}</li>
          </ul>
          ...
          <p>If you're having trouble getting the {{ $page.files.name }} extractor to work, look for an <a
